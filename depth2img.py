...
def make_batch_sd(
    ...
    if useDI:
        image_np = np.array(di_img)
        image_tensor = torch.from_numpy(image_np)
        image_float_tensor = image_tensor.to(dtype=torch.float32) / 127.5 - 1.0
        image_d = image_float_tensor # convert to float
    ...
    if useDI:
        ...
        batch["jpg"] = rearrange(batch["jpg"], 'h w c -> 1 c h w')
        batch["jpg"] = repeat(batch["jpg"].to(device=device), "1 ... -> n ...", n=num_samples)
        batch["midas_in"] = repeat(torch.from_numpy(batch["midas_in"][None, ...]).to(device=device), "1 ... -> n ...",
                                   n=num_samples)
...
def predict(input_image,dm_image, prompt, steps, num_samples, scale, seed, eta, strength):
    global di_img
    di_img= dm_image
...
d_ckpt = "midas_models/512-depth-ema.ckpt"
yaml = "stablediffusion/configs/stable-diffusion/v2-midas-inference.yaml"
sampler = initialize_model(yaml, d_ckpt)
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            with gr.Row():
                ...
                with gr.Column(min_width=100):
                    dm_image = gr.Image(source='upload', type="pil",label="Custom Depth")
            prompt = gr.Textbox(label="Prompt")
            with gr.Row():
                with gr.Column(min_width=100,scale=0.5):
                    use_DI = gr.Checkbox(value=False,label="use Depth")
    ...
    run_button.click(fn=predict, inputs=[
                     input_image,dm_image, prompt, ddim_steps, num_samples, scale, seed, eta, strength], outputs=[gallery])
    use_DI.change(fn=chooseDM,inputs=[use_DI],outputs=[])
